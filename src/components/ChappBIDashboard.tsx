...
const ChappBIDashboard = () => {
  ...
  const periods = [
    { value: 'week', label: t('bi.period.week') },
    { value: 'month', label: t('bi.period.month') },
    { value: 'year', label: t('bi.period.year') }
  ];

  const regions = [
    { value: 'all', label: t('bi.region.all') },
    { value: 'europe', label: t('bi.region.europe') },
    { value: 'america', label: t('bi.region.america') }
  ];
        ...
        <div className="max-w-7xl mx-auto animate-on-scroll">
          ...
          <div className="card-glass-dark p-6 lg:p-8">
            ...
            <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-8">
              <div>
                <h3 className="text-heading-xl text-chapp-white mb-2">{t('bi.dashboard.title')}</h3>
                <p className="text-chapp-gray-400">{t('bi.dashboard.subtitle')}</p>
              </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 bg-chapp-white/5 backdrop-blur-sm border border-chapp-white/10 rounded-2xl p-6">
                <div className="flex items-center justify-between mb-6">
                  <h4 className="text-heading-md text-chapp-white">{t('bi.chart.sales')}</h4>
              ...
              <div className="bg-chapp-white/5 backdrop-blur-sm border border-chapp-white/10 rounded-2xl p-6">
                <h4 className="text-heading-md text-chapp-white mb-6">{t('bi.chart.channels')}</h4>
                ...
                <div className="space-y-3">
                  {[
                    { label: t('bi.channel.online'), value: '67%', color: 'bg-chapp-accent-blue' },
                    { label: t('bi.channel.retail'), value: '25%', color: 'bg-chapp-accent-blue/60' },
                    { label: t('bi.channel.partner'), value: '8%', color: 'bg-chapp-accent-blue/30' }
                  ].map((channel, index) => (
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center mt-8 pt-6 border-t border-chapp-white/10">
              <p className="text-chapp-gray-400 text-body-sm">
                {t('bi.lastUpdate')}: {new Date().toLocaleString()}
              </p>
              <div className="flex items-center gap-2 mt-4 sm:mt-0">
                <div className="w-2 h-2 bg-green-400 rounded-full animate-pulse"></div>
                <span className="text-chapp-gray-300 text-body-sm">{t('bi.realTime')}</span>
              </div>
            </div>
          </div>
